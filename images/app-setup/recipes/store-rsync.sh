...
# category.name.zh: 备份
# order: 14
# summary: Where backups go: a NAS, or another machine you own, over SSH. No password — a key.
# summary.zh: 备份存到哪：一台 NAS，或者另一台你自己的机器，走 SSH。不用密码，用密钥。
# includes: rsync, an SSH key made for this, and a pinned host key
# includes.zh: rsync、为此生成的 SSH 密钥、以及固定下来的主机指纹
# disk: 10M
# memory: 16M
...
#
# rsync rather than scp because it resumes a transfer that died at 90% and
# sends only what changed. scp exists as a separate card for the storage boxes
# that give you sshd and no rsync binary at all.
. /usr/lib/app-setup/common.sh

KEY="$APP_SETUP_CONF/secrets/backup_ed25519"
KNOWN="$APP_SETUP_CONF/secrets/backup_known_hosts"
STAMP="$APP_SETUP_STATE/backup/store-rsync.ok"

PKGS="rsync openssh-client"
PKGS_rpm="rsync openssh-clients"
PKGS_apk="rsync openssh-client"

# user@host:/path -> the three pieces. The path is the *base*; the folder the
# caller passes is made under it.
rs_user_host() { printf '%s' "${1%%:*}"; }
rs_base()      { case "$1" in *:*) printf '%s' "${1#*:}" ;; *) printf '' ;; esac; }
...
rs_path() {  # rs_path <folder> [name]
	local _b
	_b="$(rs_base "$(param target)")"
	[ -n "$_b" ] || _b=.
...
your target. From this machine, if you can already log in with a password:
    ...
    ssh-copy-id -i $KEY -p $(param port 22) $(rs_user_host "$(param target)")
...
container cannot use it for anything but writing backups.

    command="rrsync $(rs_base "$(param target)")",restrict $(cat "$KEY.pub")
...
rs_pin_host() {
	local _h _p
	_h="$(rs_user_host "$(param target)")"; _h="${_h#*@}"
	_p="$(param port 22)"
...
do_put() {    # do_put <folder> <localfile>
	...
	rs_ensure_key
	do_mkdir "$1" || return 1
	rsync -a --partial --append-verify -e "$(rs_ssh_cmd)" \
		"$2" "$(rs_user_host "$(param target)"):$(rs_path "$1" "$_n")" ||
		{ err "rsync failed"; return 1; }
}

do_get() {    # do_get <folder> <name> <localfile>
	rs_ensure_key
	rsync -a --partial -e "$(rs_ssh_cmd)" \
		"$(rs_user_host "$(param target)"):$(rs_path "$1" "$2")" "$3" ||
...
do_ls() {     # do_ls <folder>
	rs_ssh "ls -1 '$(rs_path "$1")' 2>/dev/null" | sort
}
...
do_test() {
	...
	rs_ssh "rm -rf -- '$(rs_path "$_f")'" >/dev/null 2>&1 || true
	rm -rf "$_d"
...
do_install() {
	step "installing rsync and an ssh client"
	pkg_install $(pmv PKGS) || die "could not install rsync here"
	rs_ensure_key
	chmod 600 "$APP_SETUP_CONF/params/store-rsync.conf" 2>/dev/null || true
	...
	save_note store-rsync <<EOF
Backup destination — rsync over SSH
...
rsync 备份存储源
  ...
  它是什么
    把打包好的备份用 rsync 通过 SSH 送到另一台机器上 —— 一台 NAS，或者
    任何一台你自己的、开着 sshd 的机器。不用密码，用密钥。
  ...
  为什么是 rsync 不是 scp
    传到 90% 断了能续，而且只传变化的部分。对面没有 rsync 只有 sshd 的
    话，用 SCP 那张卡。
...
rsync backup destination
  ...
  What it is
    Sends packed backups over SSH with rsync to another machine — a NAS, or
    anything of your own running sshd. No password: a key.
  ...
  Why rsync rather than scp
    It resumes a transfer that died at 90% and only sends what changed. If the
