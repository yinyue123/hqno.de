...
（面板界面和 API 那两条路的默认值相反，是 SNI 直通。第 6 节讲这个区别为什么要紧。）
...
前面八节讲的都是用户态的东西：一个普通程序，拿着一个普通 socket ——
