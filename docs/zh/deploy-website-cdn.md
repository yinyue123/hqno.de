...
**不想手打八十行的话，有现成的。** app-setup 里就有这一整步 —— 网页服务器那一栏
里的 `cdn`。装上，填三个字段：
...
| 源站端口 | `443` |
| 本机域名 | `www.example.com,example.com` |

它照着这三个答案，生成的就是这一节要写的那个文件。默认只缓存静态文件 ——
图片、视频、CSS、JS —— 页面、PHP、接口一律直接转给源站，这个设置漏不出
任何属于某个人的东西；要不要连页面一起缓存是另一个决定，`app-setup docs cdn`
里写了改之前该先确认什么。生成的文件末尾还注释了八段示例：接口不缓存、
视频切片、WordPress 和商城、防盗链。

这一节剩下的部分，是同一份配置手写一遍。不管你用哪种方式装的，都值得读一遍 ——
§10 里会出的问题，每一条都对应下面的某一行。
