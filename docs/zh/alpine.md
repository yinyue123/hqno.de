...
Alpine 是小的那个。用它做出来的容器，文件系统 36 MB，Debian 是 164 MB；
空转时占大约 1 MB 内存。也就是说，同样一台盒子，硬盘和内存留给你真正要跑的
东西的部分更多。
...
## 1. 选 Alpine 还是 Debian

在本面板发布的镜像上，各开一个全新容器实测：

| | Alpine 3.24 | Debian 13 |
|---|---|---|
| 什么都没装时的文件系统 | **36 MB** | 164 MB |
| 空转时占用的内存 | **约 1 MB** | 约 10 MB |
| 包管理器 | `apk` | `apt` |
| init | busybox init + OpenRC | systemd |
| C 库 | musl | glibc |
| 日志 | `/var/log` 下的文件 | journald，外加文件 |

换成你真正关心的说法：一个卖 1 GB 硬盘的容器，Alpine 给你**多留 128 MB**；
一个卖 256 MB 内存的容器，光是开机这一项的差别就是你全部内存的十分之一。
