...
代价是体积。在你断定它无所谓之前，值得先看看真实数字。
各开一个全新容器、什么都不装、空转着实测：

| | Debian 13 | Alpine 3.24 |
|---|---|---|
| 文件系统 | **164 MB** | 36 MB |
| 空转时算进你内存限额的量 | **105 MB** | 15 MB |
| —— 其中真正拿不回来的 | **10 MB** | 0.9 MB |
| 跑着的进程数 | 7 | 3 |

那 105 MB 里绝大部分是页缓存，一有东西要用内核立刻还回来；
10 MB 才是真的没了。它花在哪：`systemd` 2.8 MB、`journald` 1.1 MB、
`logind` 1.1 MB、`rsyslogd` 0.6 MB、`dbus` 0.5 MB、`agetty` 0.2 MB、
`cron` 0.2 MB。硬盘那边，20.5 MB 是 `apt` 的包索引，5.9 MB 是 `dpkg`
的数据库 —— 这两样都不是你装的。

内存 1 GB、硬盘 10 GB 的容器上，这些全是噪音，闭着眼睛选 Debian 就好。
卖 256 MB 内存或者 1 GB 硬盘的容器上，它是真的，
[使用 Alpine](alpine.md) 第 1 节就是替另一边说话的那一页，
同样这些数字在那边拆得更细。
...
journal 自己是有上限的，但那个上限不是谁替容器挑的：
它拿**文件系统的 10%**。在量这些数字的那个容器上，
这个天花板是 **1.9 GB**；而第一次开机两分钟后，你什么都还没跑，
它已经 8 MB 了。

```sh
journalctl --disk-usage         # 它现在占了多少
journalctl --vacuum-size=100M   # 立刻砍到这个大小
```

要让它一直有效，在 `/etc/systemd/journald.conf` 里已有的 `[Journal]` 段下
加一行（这个文件里的东西出厂时全是注释掉的）：
...
然后 `systemctl restart systemd-journald`。它会在自己的第一条日志里
把新的天花板说出来 —— `System Journal … is 8M, max 100M`。
硬盘小的容器上，这件事值得第一天就做：
100 MB 的 journal 对你可能跑的任何东西来说都还是好几周的历史，
而这就是「日志装得下」和「日志吃掉你买的十分之一」之间的差别。
