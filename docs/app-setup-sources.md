...
draws one card per script, and runs the script when you press a key. So adding
...
That is a complete, working card with Install, Remove and Docs buttons. Install
and remove are inherited — `PKGS` is enough for anything that is one package
and no service.
...
`/usr/local/etc/app-setup/nginx.sh` replaces the shipped `nginx` card without
...
draw a card. That is deliberate — drawing the catalogue must not execute forty
...
| `name` | no | the title on the card. Defaults to the id |
...
| `summary` | no | two lines of prose on the card. Say what it is *for* |
...
| `ports` | no | what it listens on, for the card |
| `requires` | no | what it needs, for the card. Prose, not a dependency solver |
...
`disk` and `memory` earn their place: the card turns them red when this machine
...
- Listing more than one is allowed: `category: dev, web` puts a card on both
  tabs.
...
|---|---|---|
| `do_install` | the Install button | install `PKGS`, enable and start `SERVICE` |
| `do_uninstall` | the Remove button | stop and disable `SERVICE`, remove `PKGS` |
| `do_start` | the Start button | `svc_start` |
| `do_stop` | the Stop button | `svc_stop` |
| `do_restart` | `app-setup restart` | stop then start |
| `do_enable` | the Boot tick | `svc_enable` |
...
every card on the screen, repeatedly.
...
| Key | Effect |
|---|---|
| `detail=` | replaces the `includes` line on the card. Put the version here |
...
**It must be fast.** app-setup kills it after eight seconds and shows the card
...
app-setup list                  # is your card there, in the right category?
...
The screenshot subcommand is how the card is checked at every width without a
terminal:
...
do_install() {
	# git itself is a package; gitea is not. One card, both handled.
	pkg_install git
...
a card in a menu has not agreed to run arbitrary code from a third party; tell
them that is what this one does.

**`do_status` is on a timer.** Eight seconds, and it runs for every card on
...
entirely — in that case as four unrelated cards claiming PHP was missing.
